function warn_premade_defaults(defaults::Dict, specs::Dict, prefix::String = "")
    ...
    for (specs_key, specs_value) in specs
        ...
        if !(specs_key in keys(defaults))
            #Warn them
            @warn "$prefix a key $specs_key was set by the user. This is not valid for this agent, and is discarded"
        end
    end
end